...
GRAPH_WINDOW_KOEFS=(0.15,0.15,0.7,0.7)
...
for i in range(-100,800):
    for j in range(10):
        for ij in range(10):
            #for iji in range(10):
                xs.append(float(i+j/10+ij/100))#+iji/1000))
lins=1
lins_cst=1
graphic1=get_graphic_points(test,x=xs,x_limits=[-2000,2000],name="sinX",lines=lins)
graphic2=get_graphic_points(test2,x=xs,x_limits=[-2000,2000],lines=lins,settings=((200,0,0),POINT_RADIUS,LINE_WIDTH),name="cosX")
graphicCustom=custom_func_points(lines=lins_cst,settings=((0,200,0),7,6),name="Custom")
graphic3=get_graphic_points(test3,x=xs,x_limits=[START_FROM[0],RESOLUTION[0]],lines=lins,settings=((0,0,250),POINT_RADIUS,LINE_WIDTH),name="x**2")
graphic4=get_graphic_points(test4,x=xs,x_limits=[START_FROM[0],RESOLUTION[0]],lines=lins,settings=((100,125,4),POINT_RADIUS,LINE_WIDTH),name="x**3")
graphic5=get_graphic_points(test5,x=xs,x_limits=[START_FROM[0],RESOLUTION[0]],lines=lins,settings=((100,25,140),POINT_RADIUS,LINE_WIDTH),name="sqtr(x)")
graphic6=get_graphic_points(test6,x=xs,x_limits=[START_FROM[0],RESOLUTION[0]],lines=lins,settings=((25,124,90),POINT_RADIUS,LINE_WIDTH),name="|x|")
points_x,points_y=numbers_gen(pause=PAUSE)
GRAPHICS["SinX"]=graphic1
GRAPHICS["CosX"]=graphic2
# GRAPHICS["x2"]=graphic3
# GRAPHICS["fnc"]=graphic4
# GRAPHICS["fnc1"]=graphic5
# GRAPHICS["fnc2"]=graphic6
GRAPHICS["CUSTOM"]=graphicCustom
#print(graphic)
total_points=0
...
while True:
    ...
    NAME_GRAPHICS_CORD=[(0,RESOLUTION[1]*GRAPH_WINDOW_KOEFS[1])]
